...
const translations: Translations = {
  ...
  opencodeDesc: { zh: "使用 OpenCode 时，PageSprite 会调用 opencode run --dir {dir} --format json <prompt> 来生成代码，提示词作为最后参数传入。Agent 会修改工作目录中的 index.html 文件，生成结果从该文件读取。", en: "PageSprite calls opencode run --dir {dir} --format json <prompt> to generate code. The agent modifies index.html in the work directory and reads the result from it." },
  claudeDesc: { zh: "使用 Claude Code 时，PageSprite 会调用 claude -p <prompt> 来生成代码，提示词作为最后参数传入。Agent 会修改工作目录中的 index.html 文件，生成结果从该文件读取。", en: "PageSprite calls claude -p <prompt> to generate code. The agent modifies index.html in the work directory and reads the result from it." },
